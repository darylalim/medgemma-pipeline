...
- **`tests/test_streamlit_app.py`** — pure helpers (no Streamlit/model): `load_ct_volume` runs against real in-memory DICOMs; `load_wsi_patches` against a `_FakeSlide` mock. Plus real-asset guards that catch upstream/config drift:
  ...
  - `TestThemeConfig` (`.streamlit/config.toml`) · `TestHooksConfig` (`.claude/settings.json`) · `TestCiWorkflow` (`.github/workflows/ci.yml`).
...
Drive the CXR tab (attach the sample via `input[type="file"]`, plain-analysis prompt, Run, await `Response`), hide the dev chrome (`stToolbar`/`stHeader`/fullscreen buttons), crop to the X-ray→`Response` region, downscale to ~1200px and save as **WebP** with Pillow (~5× smaller than PNG here; GitHub renders it). Three traps:

- **Localization boxes are unreliable on this 4B model** — don't use them for the hero.
- **Force `color_scheme="dark"`** — headless Chromium defaults to light and the app follows `prefers-color-scheme`.
